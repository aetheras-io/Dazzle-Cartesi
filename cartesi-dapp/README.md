...
cartesi-dapp is a customized DApp written in Python, which originally resembles the one provided by the sample [Echo Python DApp](https://github.com/cartesi/rollups-examples/tree/main/echo-python).
...
```shell
DAPP_NAME=cartesi-dapp docker compose -f ./deploy-testnet.yml up
```

This will create a file at `./deployments/<network>/cartesi-dapp.json` with the deployed contract's address.
Once the command finishes, it is advisable to stop the docker compose and remove the volumes created when executing it.

```shell
DAPP_NAME=cartesi-dapp docker compose -f ./deploy-testnet.yml down -v
```
...
```shell
DAPP_NAME=cartesi-dapp docker compose --env-file ./env.<network> -f ./docker-compose-testnet.yml -f ./docker-compose.override.yml up
```

Which, in the case of Sepolia, would be:

```shell
DAPP_NAME=cartesi-dapp docker compose --env-file ./env.sepolia -f ./docker-compose-testnet.yml -f ./docker-compose.override.yml up
```
...
```shell
yarn start input send --payload "Hello there" --addressFile path/to/cartesi-dapp/deployments/<network>/cartesi-dapp.json
```
